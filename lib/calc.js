const runcurve = {
    ...
    getSpeed: (route, vehicle, startStaIndex) => {
        ...
        let resultSpeed = new Array(endLength - startLength); // km/h
        ...
        // 駅最高速度を代入
        resultSpeed.fill(route.stations[startStaIndex].maxSpeed);
        ...
        { // 制限速度
            for (let i = 0; i < route.limitSpeeds.length; i++)
            {
                ...
                { // 開始地点から終了地点までを計算
                    if (resultSpeed[j - startLength] >= route.limitSpeeds[i][2])
                        resultSpeed[j - startLength] = route.limitSpeeds[i][2];
                }
            }
        }
        ...
        {
            let before = resultSpeed[0];
            for (let i = 0; i < resultSpeed.length; i++)
            {
                if (before != resultSpeed[i])
                {
                    curveChangePoint.push([i, resultSpeed[i]]);
                    before = resultSpeed[i];
                }
            }
        }

        { // 惰行パターン
            let cChangePoint = [ ...curveChangePoint, [resultSpeed.length, 0] ];
            ...
            for (let i = 0; i < curveChangePoint.length; i++)
            {
                ...
                for (let j = 0; j < (cChangePoint[i+1][0] - cChangePoint[i][0]); j++ )
                {
                    ...
                    resultSpeed[j+cChangePoint[i][0]] = speed;
                    previous = speed;
        ...
        { // 駅減速
            let i = 1;
            let previous = 0;
            resultSpeed[resultSpeed.length-1] = 0;
            while (true)
            {
                if (i > resultSpeed.length)
                    break;
                let speed = runcurve.getDecelBeforeSpeed(previous, vehicle, curve[i], gradient[i], tunnel[i]);
                if (previous > resultSpeed[resultSpeed.length-i])
                    break;
                resultSpeed[resultSpeed.length-i] = speed;
                previous = speed;
                i++;
            }
        }
